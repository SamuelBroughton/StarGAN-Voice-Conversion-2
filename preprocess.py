...
if __name__ == '__main__':
    ...
    dataset_using_default = dataset_using_possibilities[0]
    needs_sampling_default = 1 # TODO: change to str2bool
    ...
    parser.add_argument("--dataset_using", type = str, default = dataset_using_default, help = "VCTK or VCC2016.")
    parser.add_argument("--needs_sampling", type = int, default = needs_sampling_default, help = "Sample on/off.")
    ...
    needs_sampling = argv.needs_sampling
    ...
    # Only sample if not 16k, e.g. VCC2016 data
    if needs_sampling == 1:  # if true
        # The original wav in VCTK is 48K, so we want to resample to 16K
        resample_to_16k(origin_wavpath, target_wavpath, num_workers=num_workers)

    speaker_used = []
    if dataset_using == dataset_using_possibilities[0]:
    ...
    elif dataset_using == dataset_using_possibilities[1]:
        speaker_used = ['sf1', 'sf2', 'sf3', 'sm1', 'sm2', 'tm1', 'tm2', 'tm3', 'tf1', 'tf2']
